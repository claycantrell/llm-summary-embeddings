...
Prior work has explored LLM-based rewriting for retrieval, embedding quality, and controllable clustering, and has tested summarization in clustering pipelines with mixed results. However, we are not aware of prior work that isolates generic abstractive summarization as a pre-embedding transformation and analyzes its effect through direct measures of embedding-space geometry, nor of work that establishes clear boundary conditions for when summarization improves versus degrades clustering on noisy versus structured text.
...
Prior work has established that LLM-based rewriting and preprocessing can improve retrieval, embedding quality, and controllable clustering. However, we are not aware of prior work that isolates generic abstractive summarization as a pre-embedding transformation and analyzes its effect through direct measures of embedding-space geometry---such as centroid separation and within/between distance ratios---together with clear boundary conditions for when summarization improves versus degrades clustering on noisy versus structured text. This paper connects LLM rewriting with embedding-space analysis by showing how summarization changes the recoverability of unsupervised structure, and when that change is beneficial versus harmful.
